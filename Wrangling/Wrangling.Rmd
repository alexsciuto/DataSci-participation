...
```{r}
dplyr::select(gapminder, c(year, lifeExp, country))
```
...
```{r}
# This will work:
dplyr::select(gapminder, country, continent, year, lifeExp)

# Better way:
dplyr::select(gapminder, c(country:lifeExp))
...
```{r}
dplyr::select(gapminder, -lifeExp)
```
...
```{r}
# compare
dplyr::select(gapminder, cont = continent)

dplyr::rename(gapminder, cont = continent)
```
...
dplyr::select(mtcars, disp, everything())
dplyr::rename(mtcars, milesPerGallon = mpg)
```
...
```{r}
# you can sort rows (observations)
...
select(gapminder, c(year, lifeExp)) %>%
...
- GDP by multiplying GPD per capita with population (`grossPDP`), and
- GDP in billions, named (`gdpBill`), rounded to two decimals.

```{r}
gapminder %>%
  mutate(grossGDP = gdpPercap * pop, gdpBill = round(grossGDP/1000000000), digits = 2)



gapminder %>%
  mutate(grossGDP = gdpPercap * pop, gdpBill = ifelse(grossGDP > pop, "good", "bad"))
...
```{r}
gapminder %>%
  transmute(country, gdpBill = (pop * gdpPercap/100000000) %>% round (2))
```
...
replace life expectancy for Banfladesh with 'NA'

```{r}
gapminder %>%
  mutate(lifeExp = if_else(country == "Canada" & year == 1952, 
                           60, 
                           lifeExp)) %>% dplyr::filter(country == "Canada")

gapminder %>%
  mutate(lifeExp = if_else(country != "Bangladesh",
                           lifeExp, 
                           NA_real_)) %>% dplyr::filter(country == "Bangladesh")
